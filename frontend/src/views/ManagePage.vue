...
import {
  Search, Refresh, Plus, Download, Delete, Edit, View, MapLocation,
...
import type { CorrosionUnitInput, InspectionRecord, CorrosionUnit, Pipeline } from '@/types/models'
...
const recordForm = reactive<Partial<InspectionRecord> & { id?: number }>({
  unit_id: undefined, item_code: 'PIPE_GROUND_POTENTIAL',
  status: 'pending', measured_value: 0, unit: '', inspector: '',
  inspection_date: new Date().toISOString(), result_summary: '',
})
...
function progressOf(unitId: number): number {
  ...
  const codes = new Set(recs.map((r) => r.item_code))
  return codes.size / 7  // 7 项检测
}
...
function openRecordDialog(rec?: InspectionRecord) {
  if (rec) {
    Object.assign(recordForm, { ...rec })
  } else {
    Object.assign(recordForm, {
      id: undefined, unit_id: store.units[0]?.id, item_code: 'PIPE_GROUND_POTENTIAL',
      status: 'pending', measured_value: 0, unit: '', inspector: '',
      inspection_date: new Date().toISOString(), result_summary: '',
    })
  }
...
async function submitRecord() {
  if (!recordForm.unit_id) { ElMessage.warning('请选择单元'); return }
  try {
    const { id, ...data } = recordForm
    if (id) {
      await recordsApi.update(id, data as any)
      ElMessage.success('已更新')
    } else {
...
<template>
  <div class="page-container">
    ...
    <el-tabs v-model="activeTab" class="mg-tabs" @tab-change="resetPage">
      ...
      <el-tab-pane :label="`📋 检测记录 (${stats.records.total})`" name="records">
        <div class="mg-toolbar">
          ...
          <el-button :icon="Plus" type="primary" @click="openRecordDialog()">新增记录</el-button>
          <div class="mg-spacer"></div>
          <el-button :icon="Download" @click="exportCurrentTab">导出当前页</el-button>
        </div>
        <el-table :data="pagedRecords" stripe border>
          ...
          <el-table-column label="操作" width="100" fixed="right">
            <template #default="{ row }">
              <el-button size="small" :icon="Edit" link type="primary" @click="openRecordDialog(row)" />
    ...
    <!-- 检测记录 Dialog -->
    <el-dialog v-model="recordDialogVisible" :title="recordForm.id ? '编辑检测记录' : '新增检测记录'" width="560px">
      <el-form :model="recordForm" label-width="100px">
        <el-form-item label="腐控单元" required>
          <el-select v-model="recordForm.unit_id" style="width:100%">
            <el-option v-for="u in store.units" :key="u.id" :label="u.name" :value="u.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="检测项" required>
          <el-select v-model="recordForm.item_code" style="width:100%">
            <el-option v-for="i in ITEMS" :key="i.code" :label="i.name" :value="i.code" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="测量值">
          <el-input-number v-model="recordForm.measured_value" :precision="3" style="width:60%" />
          <el-input v-model="recordForm.unit" placeholder="单位" style="width:35%;margin-left:5%" />
        </el-form-item>
        <el-form-item label="检测员"><el-input v-model="recordForm.inspector" /></el-form-item>
        <el-form-item label="摘要"><el-input v-model="recordForm.result_summary" type="textarea" :rows="2" /></el-form-item>
      </el-form>
      <template #footer>
