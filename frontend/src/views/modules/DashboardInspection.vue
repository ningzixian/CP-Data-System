...
function setRef(key: string) {
  return (el: any) => {
    ...
    if (!element) return
    if (refs[key] && refs[key] !== element) resizeObserver?.unobserve(refs[key])
    refs[key] = element
...
function renderItemStatus() {
  ...
  if (!el) return
  if (!charts.itemStatus) charts.itemStatus = echarts.init(el)
  // 按检测项统计：通过/异常
...
function renderItemException() {
  ...
  if (!el) return
  if (!charts.itemException) charts.itemException = echarts.init(el)
...
function renderInspector() {
  ...
  if (!el) return
  if (!charts.inspector) charts.inspector = echarts.init(el)
...
function renderCommunityBar() {
  ...
  if (!el) return
  if (!charts.communityBar) charts.communityBar = echarts.init(el)
...
function renderExceptionInspector() {
  ...
  if (!el) return
  if (!charts.exceptionInspector) charts.exceptionInspector = echarts.init(el)
...
onMounted(() => {
  ...
  Object.values(refs).forEach((element) => resizeObserver?.observe(element))
