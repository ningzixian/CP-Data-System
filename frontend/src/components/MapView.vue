...
import L, { type Map as LeafletMap, type LayerGroup, type Marker, type Renderer } from 'leaflet'
...
let pipeRenderer: Renderer | null = null
...
const PIPE_STYLE_DEFAULT = { color: '#67c23a', weight: 3, opacity: 0.75, lineCap: 'round', lineJoin: 'round' } as const
const PIPE_STYLE_ACTIVE = { color: '#8B4513', weight: 6, opacity: 1, lineCap: 'round', lineJoin: 'round' } as const

function applyPipeStyle(line: any, active: boolean) {
  line.setStyle(active ? PIPE_STYLE_ACTIVE : PIPE_STYLE_DEFAULT)
}
...
function clearSelection() {
  ...
  if (selected.kind === 'pipe') {
    ...
    applyPipeStyle(selected.item, false)
  } else {
...
let pendingCommunityRemoveTimer: ReturnType<typeof setTimeout> | null = null
let detailFadeTimer: ReturnType<typeof setTimeout> | null = null
 ...
 *  - 出场可按设施类型选择是否跑快速 opacity 渐隐,见 runLeaveAnimation
 *  - animate=false:showDetailView 用,避免和细节视图整体渐现叠加
 */
 ...
 *  - animate: 是否允许隐藏渐隐动画
 *  - 隐藏动画只保留 unit / regulator:
 *    pipe 是 Canvas,没有可渐隐的 path DOM; joint / inlet 数量容易变多,直接 remove 更轻。
 */
function applyFacilityVisibility(opts: { animate?: boolean } = {}) {
  ...
  toggleLayer(unitPolyLayer, vis.unit, { leave: animate })
  toggleLayer(progressDefaultLayer, vis.unit, { leave: animate })
  toggleLayer(progressHighlightLayer, vis.unit, { leave: animate })
  // 其余四种独立切换
  toggleLayer(pipeLayer, vis.pipe, { leave: false })
  toggleLayer(jointLayer, vis.joint, { leave: false })
  toggleLayer(regulatorLayer, vis.regulator, { leave: animate })
  toggleLayer(inletLayer, vis.inlet, { leave: false })
}
...
/** 管线交互:click 选中 + hover 高亮
 *  - 管线使用 Canvas renderer,没有可操作的 SVG path DOM
 *  - hover/selected 样式统一用 Leaflet setStyle 更新,避免依赖 CSS :hover
 *  - 互斥:clearAll() 清掉旧的设施 + 控制单元选中
 */
function setupPipeInteraction(line: any) {
  function bringToFront() {
    line.bringToFront?.()
    // 兼容 SVG renderer 的旧路径;Canvas 下没有 _path/_rootGroup,这里自然跳过。
...
function renderPipes() {
  ...
  pipes.forEach((p) => {
    ...
    const line = L.polyline(p.coords.map(([lng, lat]) => [lat, lng]), {
      ...PIPE_STYLE_DEFAULT,
      renderer: pipeRenderer ?? undefined,
...
function showDetailView() {
  ...
  if (mapRef.value) {
    ...
    if (detailFadeTimer !== null) clearTimeout(detailFadeTimer)
    detailFadeTimer = setTimeout(() => {
      mapRef.value?.classList.remove('detail-fading')
...
defineExpose({ zoomToCommunityView, invalidate: () => map?.invalidateSize() })
...
onMounted(async () => {
  ...
  // 管线数量较大时用 Canvas renderer，避免每段管线生成一个 SVG path DOM 节点。
  pipeRenderer = L.canvas({ padding: 0.5 })
...
// 普通切换单元时地图容器尺寸不变,不要 invalidateSize,否则会强制布局并卡住飞行动画。
watch(
  () => store.selectedUnit,
  (u) => {
    if (!map || !u) return
    ...
    const dy = targetPoint.y - centerPoint.y
    const pixelDistance = Math.sqrt(dx * dx + dy * dy)
    // 已经在屏幕中心 ± 50 像素内且已经足够放大 → 不动。
    // 如果还在小区概览 zoom,即使中心接近也必须放大到单元级。
    if (pixelDistance < 50 && map.getZoom() >= 18) return
    map.stop()
    map.flyTo(targetCenter, 18, { duration: 0.6 })
  },
)
...
onBeforeUnmount(() => {
  ...
  if (detailFadeTimer !== null) {
    clearTimeout(detailFadeTimer)
    detailFadeTimer = null
  }
  map?.remove()
  map = null
