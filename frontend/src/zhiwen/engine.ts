...
function matchDataset(text: string): string {
  // “进度/完成度 + 单元”描述的是检测腐控单元，不是 GIS 控制面。
  if (/单元/.test(text) && /(进度|完成率|完成度|已完成|未完成|待开始|进行中|100%)/.test(text)) return 'units'
  // 只有时间、异常等业务词时，默认查询检测记录，避免错误回落到管线。
  if (/(本周|这周|上周|本月|这个月|上月|今天|今日|最近\s*\d+\s*(天|个?月))/.test(text)
      && /(异常|不合格|通过|新增|完成|检测|录入)/.test(text)) return 'records'
...
function hasRealDatasetMatch(text: string): boolean {
  if (/单元/.test(text) && /(进度|完成率|完成度|已完成|未完成|待开始|进行中|100%)/.test(text)) return true
  if (/(本周|这周|上周|本月|这个月|上月|今天|今日|最近\s*\d+\s*(天|个?月))/.test(text)
      && /(异常|不合格|通过|新增|完成|检测|录入)/.test(text)) return true
  for (const k of Object.keys(DATASET_KEYWORDS)) {
...
function isAvgIntent(text: string) {
  if (!/平均|均值|avg|mean/i.test(text)) return false
  // 管线号、编号、代号等是字符串标识，不能作为数值参与平均值计算。
  const identifier = '(?:管线号|管线编号|编号|代号|设备号|设施号|ID|id)'
  const average = '(?:平均|均值|avg|mean)'
  if (new RegExp(`${identifier}.{0,4}${average}`, 'i').test(text)) return false
  if (new RegExp(`${average}.{0,4}${identifier}`, 'i').test(text)) return false
  return true
}
...
/** 用户只询问某个可识别设施的位置，不附带统计或详情意图。 */
export function isLocationOnlyQuery(text: string): boolean {
  const value = text.trim()
  if (!/(在哪|在哪儿|在哪里|位置)(\?|？|。|；|;|,|，|$)/.test(value)) return false
  if (/什么|哪些|几个|多少|总长|总长度|平均|分布|怎么|如何|为什么/.test(value)) return false
  return isUnitLookupQuery(value)
}
...
export function extractLookupKey(text: string): { key: string; type: 'fskz' | 'ecode' | 'building' | 'unit' | 'damage' | 'free' } | null {
  const trimmed = text.trim()

  // 0) 防腐层破损点内部定位键：DP-记录ID-点位序号。
  const damage = trimmed.match(/\b(DP-\d+-\d+)\b/i)
  if (damage) return { key: damage[1].toUpperCase(), type: 'damage' }
...
export function searchFacilityByKey(
  key: string,
  type: 'fskz' | 'ecode' | 'building' | 'unit' | 'damage' | 'free',
  data: ZhiwenData,
): Array<{
  dataset: string
  type: string
  ...
  // DP-记录ID-点位序号 → 检测记录内的防腐层破损点。
  if (type === 'damage') {
    const point = collectDamagePoints(data).find((item) => `DP-${item.uid}`.toUpperCase() === key.toUpperCase())
    if (!point) return matches
    matches.push({
      dataset: 'records',
      type: '防腐层破损点',
      name: point.name || key,
...
export function extractTopN(text: string): { n: number; sortField: 'length' | 'progress' | 'damage_count' | 'count' | 'measured_value' | 'diametero' | 'build_year'; order: 'desc' | 'asc' } | null {
  // "前N名" / "前 N 个"
  let m = text.match(/前\s*(\d+)\s*(名|个|条|台|座|根|段)/)
  if (m) return { n: parseInt(m[1]), sortField: 'count', order: 'desc' }

  // "最...的N个/条" — 需要识别排序方向
  m = text.match(/最(长|大|高|多|重|深|严|旧|老|新|早|晚|快|慢|强|弱|好|差)\s*的?\s*(\d+)\s*(个|条|台|座|根|段|口|栋|名)?/)
  if (m) {
    const word = m[1]
    const n = parseInt(m[2])
    const ascWords = ['短', '小', '低', '少', '轻', '浅', '新', '早', '慢', '弱', '差']
    ...
    else if (['旧', '老', '新', '早', '晚'].includes(word)) sortField = 'build_year'
    return { n, sortField, order }
  }

  // "最...的是哪/什么" — 无数字，n=1（"管线最旧的是哪年" → 找最早的 1 条）
  m = text.match(/最(长|大|高|多|重|深|严|旧|老|新|早|晚|快|慢|强|弱|好|差|短|小|低|少|轻|浅)\s*(的\s*)?(是|那|哪|在|多|几|多少|什)/)
  if (m) {
    const word = m[1]
    const ascWords = ['短', '小', '低', '少', '轻', '浅', '新', '早', '慢', '弱', '差']
    const order: 'desc' | 'asc' = ascWords.includes(word) ? 'asc' : 'desc'
...
export function matchTimeRange(text: string, now = new Date()): TimeRange | null {
  // 使用本地年月日，避免中国时区午夜经 toISOString() 转成前一天。
  const ymd = (d: Date) => {
...
export function matchProgressRange(text: string): { op: '>=' | '<=' | '>'; value: number } | null {
  ...
  if (m) return { op: '<=', value: parseInt(m[1]) / 100 }
  if (/(已经|已|全部)\s*100\s*%\s*(完成|完工|已完)/.test(text)) return { op: '>=', value: 1 }
  if (/进度\s*(达到|到|完成|是)\s*100\s*%?/.test(text)) return { op: '>=', value: 1 }
  return null
}

/** 常见中文姓氏，用于避免把“完成”“知道”等普通词误识别为检测员姓名。 */
...
function looksLikeChineseName(value: string): boolean {
  return value.length >= 2 && value.length <= 3 && COMMON_SURNAMES.has(value[0])
}
...
export function matchInspector(text: string): string | null {
  ...
  const NAME_BOUNDARY = '(?:做的?|检测的?|录入的?|巡检的?|施工的?|的检测|的录入|的巡检|的施工|的所有记录|的全部记录|的记录|最|$)'
...
export function compareTwoUnits(a: string, b: string, data: ZhiwenData): {
  ...
  const findUnit = (fskz: string): any => {
    ...
    if (ctrl || u) {
      // GIS 控制面提供定位坐标，腐控单元提供真实检测进度；合并后再比较。
      return {
        ...(ctrl ?? {}),
        ...(u ?? {}),
        lng: u?.lng ?? ctrl?.lng,
        lat: u?.lat ?? ctrl?.lat,
        community: u?.community || ctrl?.community || '',
        source: ctrl ? 'control' : 'unit',
      }
    }
    return null
  }
...
export function scoreConfidence(text: string): { score: number; reasons: string[] } {
  ...
  if (isProgressIntent(text) || /100%\s*(完成|完成率)/.test(text)) {
    score += 0.2; reasons.push('进度')
  }
  // 检测项关键词（管地电位等）
  ...
  if (/\d{4}\s*年\s*(以前|之前|建成)/.test(text) || /建设年代/.test(text)) {
    score += 0.3; reasons.push('年份范围')
  }
  if (/进度|完成率|完成度|未完成|进度异常/.test(text)) {
    score += 0.3; reasons.push('进度意图')
  }
  if (/(已经|已|全部)?\s*100\s*%\s*(完成|完工)|(已经|已|全部)?\s*完成\s*(的|了)/.test(text)
      || /未完成|未完工|未启动|未开始/.test(text)) {
  ...
  if (isDamagePointQuery(q)) {
    ...
    if (points.length > 0) {
      ...
      return {
        ...
        // 仅明确询问位置时留在智问页；数量、列表等查询仍展示完整回答。
        // main 同时保留 DP key 的主地图反查兼容能力。
        isLocationOnly: !isCount && /(位置|在哪|在哪里|在哪儿)/.test(q),
  ...
  if (isUnitLookupQuery(q)) {
    ...
    if (lookup) {
      ...
      if (matches.length > 0) {
        ...
        // 没找到 - 给出引导
        // 数据源缺少楼号字段时，退回到目标小区的设施清单，避免楼栋位置查询直接返回空白。
        if (lookup.type === 'building' || lookup.type === 'unit') {
          const targetCommunity = matchCommunity(q)
          const allFacilities: ReturnType<typeof searchFacilityByKey> = []
          const facilitySources = [
            { dataset: 'inlets' as const, rows: data.inlets, label: '引入口' },
            { dataset: 'joints' as const, rows: data.joints, label: '绝缘接头' },
            { dataset: 'regulators' as const, rows: data.regulators, label: '调压箱' },
          ]
          facilitySources.forEach((source) => {
            source.rows.forEach((facility) => {
              if (targetCommunity && facility.community !== targetCommunity) return
              if (!Number.isFinite(Number(facility.lng)) || !Number.isFinite(Number(facility.lat))) return
              allFacilities.push({
                dataset: source.dataset,
                type: source.label,
                name: facility.name || facility.ecode || '',
                ecode: facility.ecode || '',
                community: facility.community,
                location_desc: '',
                lng: Number(facility.lng),
                lat: Number(facility.lat),
                pipeno: facility.pipeno || '',
                raw: facility,
              })
            })
          })
          const visibleFacilities = allFacilities.slice(0, 50)
          if (visibleFacilities.length) {
            const breakdown = facilitySources.map((source) => {
              const count = visibleFacilities.filter((item) => item.dataset === source.dataset).length
              return count ? `${source.label}${count}个` : ''
            }).filter(Boolean).join('、')
            return {
              text: `“${lookup.key}”在现有数据中没有楼号字段，暂时无法精确匹配。\n\n已展示${targetCommunity || '全部小区'}的相关设施作为参考（${visibleFacilities.length} 个，${breakdown}）。\n\n可使用 FSKZ 控制单元编号或 ECODE 设施编号进一步精确查询。`,
              totalCount: allFacilities.length,
              table: {
                headers: ['名称', '类型', '小区', '编号', '管线', '经度', '纬度'],
                rows: visibleFacilities.map((item) => ({
                  名称: item.name || item.ecode,
                  类型: item.type,
                  小区: item.community,
                  编号: item.ecode,
                  管线: item.pipeno || '—',
                  经度: item.lng.toFixed(6),
                  纬度: item.lat.toFixed(6),
                })),
              },
              mapDataset: 'inlets',
              mapCommunity: targetCommunity || undefined,
              mapFocus: 'all',
              mapOverlay: visibleFacilities.map((item, index) => ({
                lng: item.lng,
                lat: item.lat,
                type: item.type,
                name: item.name || item.ecode,
                community: item.community,
                status: 'matched',
                isPrimary: index === 0,
              })),
              sql: `编号反查 fallback | key="${lookup.key}" type=${lookup.type} | 无楼号字段，列出 ${visibleFacilities.length} 个设施`,
            }
          }
        }
        return {
...
在数据库中补充更多有效数据。\n\n💡 支持的查询格式：\n- 控制单元：**FSKZ755853**\n- 设施编号：**N54R328A067** / **N79R501**\n- 楼栋引用：**1号楼** / **2号楼四单元**\n- 直接问："N54R328A067 在哪个小区？" / "1号楼有什么设施？"`,
  ...
  if (topN && topN.sortField !== 'build_year'
      && (dataset === 'pipes' || dataset === 'units' || dataset === 'inlets' || dataset === 'joints' || dataset === 'regulators')) {
    const n = topN.n
  ...
  if (dataset === 'pipes' && topN?.sortField === 'build_year') {
    ...
    return {
      ...
      sql: `建设年代 Top-${topN.n} | sort by build_year ${topN.order}`,
    }
  }
